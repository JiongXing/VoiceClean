...
5. **coremltools 输出优化陷阱**：`unroll=True` + `seq_len=1` 时，LSTM 的 `h_out` 与序列输出是同一计算节点，coremltools 的图优化 pass 会将其合并，导致 5 个输出被缩减为 3 个（丢失 h 状态）。解决方案：用 `Activation('linear')` 包裹每个状态输出创建独立节点，转换后通过 **shape 识别** 定位主输出 + **扰动测试** 确定状态映射，最后用 `ct.utils.rename_feature` 重命名为语义化名称
6. **模型加载策略**：Xcode 将 `.mlpackage` 自动编译为 `.mlmodelc` 放入 App Bundle，运行时应直接用 `MLModel(contentsOf:)` 加载，**不能**再调用 `MLModel.compileModel(at:)` 二次编译，否则会因 Manifest.json 缺失报错。代码中实现了双层回退：优先加载 `.mlmodelc`，若不存在则从 `.mlpackage` 运行时编译
...
**挑战四：coremltools 优化 pass 静默丢弃 LSTM 状态输出**

这是在 App 运行阶段才暴露的问题。模型转换看似成功，但运行时报错 `feature 'm1_state_c_0' must be of rank 2, instead got a multi-array value of rank 3`。

根因分析：`unroll=True` + `seq_len=1` 时，LSTM 的序列输出和 `h_out` 是**同一个计算节点**（对于单步序列，输出即隐藏状态）。coremltools 在 95 个优化 pass 中检测到这一冗余，将其合并为一个输出。模型定义的 5 个输出（mask + 4 个状态）被缩减为 3 个（mask + 2 个 c 状态）。Swift 代码按 `Identity, Identity_1, ...` 的固定顺序映射输出，把 mask（rank 3）误赋给了 c_0 状态变量，导致第 2 帧推理时 rank 不匹配。

解决方案分三步：

```python
# 1. 用 Activation('linear') 包裹状态输出，创建独立计算节点
h_out = Activation('linear', name=f"m1_out_h_{i}")(h_out)
c_out = Activation('linear', name=f"m1_out_c_{i}")(c_out)

# 2. 转换后通过 shape 识别主输出（mask [1,1,257] vs 状态 [1,128]）
for k, v in baseline.items():
    if len(v.shape) == 3 and v.shape[-1] == main_shape_last_dim:
        main_key = k  # 这是 mask/frame 输出

# 3. 通过扰动测试确定状态映射（哪个输出对应哪个输入）
perturbed[state_input] = np.full((1, NUM_UNITS), 5.0)
result = model.predict(perturbed)
best_match = max(state_outputs, key=lambda k: diff(result[k], baseline[k]))
```

这个方案的关键洞察：**不依赖 coremltools 内部的命名规则**（Identity 编号与 Keras 输出顺序无对应关系），而是用**运行时行为**（shape + 扰动响应）作为输出身份的判定依据。
